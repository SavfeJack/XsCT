...
{
    std::string inputStr = "C://Users//user//Desktop//temp//result_img.nii";
    std::vector<char> inputChars(inputStr.begin(), inputStr.end());
    inputChars.push_back('\0');

    char* input_name = &inputChars[0];
    const char* output_name = "C://Users//user//Desktop//temp//drr01.bmp";
    bool ok;
    bool verbose = false;
    float rx = -90.;
    float ry = 0;
    float rz = 0.; //ap0, lat-90
    float tx = 0.;
    float ty = 0.;
    float tz = -15.;
    float cx = 0.;
    ...
    if (output_name)
    {
        ...
        using ImageIOType2 = itk::PNGImageIO;
        ImageIOType2::Pointer pngImageIO = ImageIOType2::New();
