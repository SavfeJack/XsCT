...
def parse_args():
    ...
    parse.add_argument('--datasetfile', type=str, default='./data/train.txt', dest='datasetfile',
                       help='')
    parse.add_argument('--valid_datasetfile', type=str, default='./data/valid.txt', dest='valid_datasetfile',
                       help='')
...
if __name__ == '__main__':
    ...
    if args.valid_dataset is not None:
        ...
        print('valid dataset {}'.format(valid_dataset.name))
    ...
    # visualizer
    from libs.utils.visualizer import Visualizer
    ...
    for epoch in range(opt.epoch_count, opt.niter + opt.niter_decay + 1):
        ...
        for epoch_i, data in enumerate(dataloader):
            ...
            # if total_steps == 1:
            #   visualizer.add_graph(model=gan_model, input=gan_model.netG.forward())
            ...
            if epoch % opt.save_epoch_freq == 0 or epoch == 1:
                ...
                for v_i in range(len(valid_dataloader)):
                    ...
                    if v_i < opt.howmany_in_train:
                        visualizer.add_image('Test_Image', gan_model.get_current_visuals(),
                ...
                Mse_avg = float(np.average(visualizer.cache['Epoch Test_Metrics']["Mse"]))
                visualizer.add_average_scalers('Epoch Test_Metrics', None, step=epoch, write=True)
                if Mse_avg < min_Mse:
                    min_Mse = Mse_avg
                    gan_model.save_networks(epoch, total_steps, latest=True)
